...
public class ChaCha20Poly1305 {
  ...
  public static byte[][] encrypt(String message, String b64EncTextWithIV) throws Exception {
    ...
    // KeyStore.Entry entry = ks.getEntry(SHARED_KEY_ALIAS, null);
    // if (entry == null) {
    // Log.w(TAG, "No key found under alias: " + SHARED_KEY_ALIAS);
    // throw new InvalidKeyException("SecretKey must NOT be NULL");
    // }
    // if (! (entry instanceof KeyStore.SecretKeyEntry)) {
    // Log.w(TAG, "entry is not SecretKeyEntryunder alias: " + SHARED_KEY_ALIAS);
    // throw new InvalidKeyException("SecretKey must NOT be NULL");
    // }
